...
describe('GlobalError', () => {
  it('renders message and calls reset when Try again clicked', async () => {
    ...
    const { container } = render(<GlobalError error={err} reset={reset} siteInfo={{ email: 'a@x.com' }} />);
  ...
  it('shows details when toggled', async () => {
    ...
    const { container } = render(<GlobalError error={err} />);
  ...
  it('uses email when provided and shows unavailable when none provided', () => {
    const { container: c1 } = render(<GlobalError error={new Error('x')} siteInfo={{ email: 'a@x.com' }} />);
    expect(c1.querySelector('.ge-link')?.getAttribute('href')).toBe('mailto:a@x.com');

    const { container: c2 } = render(<GlobalError error={new Error('x')} siteInfo={{}} />);
  ...
  it('includes correct a11y attributes (role, aria-live) and forwards className', async () => {
    const { container } = render(<GlobalError error={new Error('boom')} className="my-app" />);
